...
if (isset($_GET['success']) && $_GET['success'] == '1') {
    $successMessage = "Sales voucher created successfully!";
}
...
// Auto-generate Sales Voucher number (prefix S)
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && empty($errors)) {
    ...
    // $debit_ledger_id = $_POST['debit_ledger_id']; // Single ledger// Cash/Bank/Party
    ...
    $credit_ledgers = $_POST['credit_ledger_id'] ?? []; // Sales ledger(s)
    ...
    foreach ($credit_ledgers as $index => $ledger_id) {
        $narr = (string)$narration[$index];
        if (empty($ledger_id) || !is_numeric($credit_amounts[$index]) || $credit_amounts[$index] <= 0) {
            $errors[] = "Invalid credit entry at row " . ($index + 1);
        }
        $total_amount += (float)$credit_amounts[$index];

        if ($ledger_id == $debit_ledger_id) {
            $errors[] = "Sales and Debit ledger cannot be the same.";
        }
    }
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            foreach ($credit_ledgers as $i => $ledger_id) {
                ...
                $new_bal = ($dc_type === 'Credit') ? $cur_bal + $amount : $cur_bal - $amount;

                $txn_stmt = $conn->prepare("INSERT INTO transactions (user_id, voucher_id, ledger_id, acc_code, transaction_type, amount, closing_balance, mode_of_payment, opposite_ledger, transaction_date, narration) 
                VALUES (?, ?, ?, ?, 'Credit', ?, ?, ?, ?, ?, ?)");
                $txn_stmt->bind_param("iiisdssiss", $user_id, $voucher_id, $ledger_id, $acc_code, $amount, $new_bal, $mode_of_payment, $debit_ledger_id, $voucher_date, $nar);
                $txn_stmt->execute();
                $txn_id = $txn_stmt->insert_id;
                $txn_stmt->close();
            ...
            $conn->commit();
            $success = "Sales voucher $voucherNumber created successfully!";
            $voucherNumber = 'S' . ($nextNum + 1);

            header("Location: sales_voucher.php?success=1");
            exit;
...
<html>
<head>
    <title>Sales Voucher - FINPACK</title>
...
<div class="voucher-container tally-style">
    ...
    <form method="POST" onsubmit="return validateForm()" id ="SalesVoucherform" class="voucher-form" autocomplete="off">
        <div class="form-row">
...
<!-- Cash/Bank Ledger for Cash/Bank/Cheque Sale -->
...
<div class="form-group">
    ...
    <table id="creditTable" style="margin-top:0px;">
        ...
        <tbody>
            <tr>
                <td>
                    <select name="credit_ledger_id[]" class="form-control" onchange="fetchBalance(this, 'credit_balance_0')" required>
                        <option value="">--Select--</option>
                        <?php 
                        $debit_ledger_id = $_POST['debit_ledger_id'] ?? 0;
                        foreach ($ledgers as $ledger): 
                            if ($ledger['acc_type'] === 'Income' && $ledger['group_id'] !== '23'): ?>
                                <option value="<?= $ledger['ledger_id'] ?>"><?= $ledger['ledger_name'] ?></option>
...
function updateDebitLedgers() {
    ...
    debitSelects.forEach(select => {
        ...
        <?php foreach ($ledgers as $ledger): ?>
            <?php if (in_array($ledger['acc_type'], ['Expense', 'Asset'])): ?>
...
window.addEventListener("load", function () {
    ...
    if (urlParams.get("success") === "1") {
        document.getElementById("SalesVoucherform")?.reset();
