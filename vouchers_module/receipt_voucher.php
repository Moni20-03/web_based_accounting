...
if ($_SERVER["REQUEST_METHOD"] == "POST" && empty($errors)){
    ...
    // Cash/Bank account – DEBIT in Receipt voucher
    ...
    if (!is_array($credit_ledgers) || count($credit_ledgers) === 0) {
        $errors[] = "Please add at least one credit entry.";
    }
...
<body>
    ...
    <input type="hidden" name="voucher_type" value="Receipt"> 
        ...
        <div id="debit_balance" class="balance-box"></div>
            </div>
...
function addRow() {
    ...
    let newTargetId = 'credit_balance_' + rowCount;
...
function updateCreditLedgers() {
    ...
    creditSelects.forEach(select => {
        ...
        const targetId = select.getAttribute('data-target') || 'credit_balance_0';
