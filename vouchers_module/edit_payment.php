...
if (!$voucher_id) {
    header("Location: payment_vouchers_list.php");
    exit;
}
...
$stmt = $conn->prepare("SELECT * FROM vouchers WHERE voucher_id = ? AND voucher_type = 'Payment'");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && empty($errors)) {
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            $narration_summary = "Payment transfer from ledger ID: $credit_ledger_id";
            ...
            foreach ($debit_ledgers as $index => $ledger_id) {
                ...
                if ($transaction_id > 0) {
                    ...
                    $log_stmt = $conn->prepare("INSERT INTO audit_logs 
                                              (user_id, table_name, record_id, action, old_value, new_value) 
                                              VALUES (?, 'transactions', ?, 'UPDATE', ?, ?)");
                    // Note: For simplicity, we're not fetching old values for each debit transaction
                    $log_stmt->bind_param("iiss", 
                        $user_id, 
                        $transaction_id, 
            ...
            $conn->commit();
            $_SESSION['success_message'] = "Payment voucher updated successfully!";
            header("Location: payment_vouchers_list.php");
            exit;
            ...
            <table id="debitTable" style="margin-top:0px;">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><input type="number" name="debit_amount[]" class="form-control" step="1" min="1" 
                    ...
                    <tr>
                        ...
                        <td><input type="number" name="debit_amount[]" class="form-control" step="1" min="1"></td>
...
function validateForm() {
    ...
    // // Validate total amount matches credit amount
    // const creditAmount = parseFloat(<?= $credit_entry ? $credit_entry['amount'] : 0 ?>);
    // if (Math.abs(totalAmount - creditAmount) > 0.01) {
    //     errors.push(`Total debit amount (${totalAmount}) must match credit amount (${creditAmount})`);
    // }
