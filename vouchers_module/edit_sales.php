...
if (!$voucher_id) {
    header("Location: sales_vouchers_list.php");
    exit;
}
...
$stmt = $conn->prepare("SELECT * FROM vouchers WHERE voucher_id = ? AND voucher_type = 'Sales'");
...
$stmt = $conn->prepare("SELECT t.*, l.ledger_name, l.acc_code, l.debit_credit, l.group_id 
                        FROM transactions t
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && empty($errors)) {
    ...
    $narration = $_POST['credit_narration'] ?? [];
    
    if ($mode_of_payment === 'Credit') {
        $debit_ledger_id = $_POST['party_ledger_id'] ?? null;
    } else {
        $debit_ledger_id = $_POST['cashbank_ledger_id'] ?? null;
    }
    ...
    if (empty($debit_ledger_id)) $errors[] = "Please select a party/cash/bank account.";

    if (count($credit_ledgers) == 0) $errors[] = "Please add at least one sales ledger entry.";
    ...
    foreach ($credit_ledgers as $index => $ledger_id) {
        ...
        if ($ledger_id == $debit_ledger_id) {
            $errors[] = "Sales and Debit ledgers cannot be the same.";
        }
    }
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            $narration_summary = "Sales made from ledger ID: $debit_ledger_id";
            ...
            $conn->commit();
            $_SESSION['success_message'] = "Sales voucher updated successfully!";
            header("Location: sales_vouchers_list.php");
            exit;
...
<html>
<head>
    <title>Edit Sales Voucher - FINPACK</title>
...
<div class="voucher-container tally-style">
    ...
    <form method="POST" id="salesVoucherForm" onsubmit="return validateForm()" class="voucher-form" autocomplete="off">
        ...
        <div class="form-row">
            <!-- Party Selection -->
            <div class="form-group" id="creditPartySection" style="<?= ($debit_entry && $debit_entry['group_id'] == '23') ? '' : 'display:none;' ?>">
                <label>Party Ledger (Customer - Sundry Debtors):</label>
                <select name="party_ledger_id" id="party_ledger" class="form-control" onchange="fetchBalance(this, 'party_balance')">
                    <option value="">--Select Party--</option>
                    <?php foreach ($ledgers as $ledger): ?>
                        <?php if ($ledger['group_id'] === '23'): ?>
                ...
                <div id="party_balance" class="balance-box">
                    <?php if ($debit_entry && $debit_entry['group_id'] == '23'): ?>
            ...
            <!-- Cash/Bank Ledger -->
            <div class="form-group" id="cashBankSection" style="<?= ($debit_entry && $debit_entry['group_id'] != '23') ? '' : 'display:none;' ?>">
                <label>Received In (Cash/Bank Ledger):</label>
                <select name="cashbank_ledger_id" id="cashbank_ledger" class="form-control" onchange="fetchBalance(this, 'cashbank_balance')">
                ...
                <div id="cashbank_balance" class="balance-box">
                    <?php if ($debit_entry && $debit_entry['group_id'] != '23'): ?>
                        Balance: <?= $debit_entry['closing_balance'] ?> (<?= $debit_entry['debit_credit'] ?>)
                    <?php endif; ?>
                </div>
            ...
            <div class="form-group">
                ...
                <select name="mode_of_payment" id="mode_of_payment" class="form-control" onchange="toggleLedgerFields(this.value);toggleRefFields(this.value);">
                    <option value="Credit" <?= ($debit_entry && $debit_entry['group_id'] == '23') ? 'selected' : '' ?>>Credit</option>
                    <option value="Cash" <?= ($debit_entry && $debit_entry['mode_of_payment'] === 'Cash') ? 'selected' : '' ?>>Cash</option>
                    <option value="Bank" <?= ($debit_entry && $debit_entry['mode_of_payment'] === 'Bank') ? 'selected' : '' ?>>Bank</option>
        ...
        <div class="form-group">
            ...
            <table id="creditTable" style="margin-top:0px;">
                <thead>
                    <tr>
                        <th width="45%">Sales Ledger (Income A/c)</th>
                    ...
                    <tr>
                        <td>
                            ...
                            <select name="credit_ledger_id[]" class="form-control" onchange="fetchBalance(this, 'credit_balance_<?= $index ?>')" required>
                                ...
                                <?php foreach ($ledgers as $ledger): ?>
                                    <?php if ($ledger['acc_type'] === 'Income' && $ledger['group_id'] !== '23'): ?>
                                        <option value="<?= $ledger['ledger_id'] ?>" 
...
document.addEventListener('DOMContentLoaded', function() {
    // Set initial state based on mode of payment
    const mode = document.getElementById('mode_of_payment').value;
    toggleLedgerFields(mode);
    toggleRefFields(mode);
});
...
function validateForm() {
    ...
    if (mode === 'Credit') {
        debitLedger = document.getElementById('party_ledger');
        if (!debitLedger.value) {
            debitLedger.classList.add('is-invalid');
            errors.push('Please select a party ledger');
        }
    } else {
        debitLedger = document.getElementById('cashbank_ledger');
        if (!debitLedger.value) {
            debitLedger.classList.add('is-invalid');
            errors.push('Please select a cash/bank ledger');
        }
    }
    
    // Validate reference fields for cheque
    if (mode === 'Cheque') {
    ...
    const creditLedgers = document.querySelectorAll('select[name="credit_ledger_id[]"]');
    let totalAmount = 0;
    ...
    creditLedgers.forEach((ledger, index) => {
        ...
        if (!ledger.value) {
            ...
            errors.push(`Row ${index + 1}: Please select a sales ledger`);
        }
        ...
        if (debitLedger && ledger.value && ledger.value === debitLedger.value) {
            ledger.classList.add('is-invalid');
            debitLedger.classList.add('is-invalid');
            errors.push(`Row ${index + 1}: Sales and Debit ledgers cannot be the same`);
        }
    });
